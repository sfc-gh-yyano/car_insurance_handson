...
ALTER SNOWFLAKE INTELLIGENCE SNOWFLAKE_INTELLIGENCE_OBJECT_DEFAULT 
  ADD AGENT SNOWFLAKE_INTELLIGENCE.AGENTS.DRIVER_INSURANCE_CHATBOT_AGENT;
...
INSURANCE_FEEDBACK（顧客フィードバック）…約10行
...
-- =========================================================
-- 5) INSURANCE_FEEDBACK（10行）
-- =========================================================
-- サンプルテーブル定義（自動車保険の顧客フィードバック）
CREATE OR REPLACE TABLE handson.car_insurance.insurance_feedback (
    feedback_id INT,
    customer_id INT,
...
-- サンプルデータ挿入
INSERT INTO handson.car_insurance.insurance_feedback VALUES
...
-- =============================================
-- ROW ACCESS POLICY セットアップ
-- POLICIES テーブルを REGISTERED_PREF で行制御
-- =============================================

-- 1. ロール作成
CREATE ROLE IF NOT EXISTS SALES_TOKYO_ROLE;
CREATE ROLE IF NOT EXISTS SALES_OSAKA_ROLE;

-- 2. 行アクセスポリシー作成
CREATE OR REPLACE ROW ACCESS POLICY HANDSON.CAR_INSURANCE.RAP_POLICIES_BY_PREF
AS (REGISTERED_PREF VARCHAR) RETURNS BOOLEAN ->
  CASE
    WHEN CURRENT_ROLE() = 'SALES_TOKYO_ROLE' AND REGISTERED_PREF = 'Tokyo' THEN TRUE
    WHEN CURRENT_ROLE() = 'SALES_OSAKA_ROLE' AND REGISTERED_PREF = 'Osaka' THEN TRUE
    WHEN CURRENT_ROLE() NOT IN ('SALES_TOKYO_ROLE', 'SALES_OSAKA_ROLE') THEN TRUE
    ELSE FALSE
  END;

-- 3. ポリシーをテーブルに適用
ALTER TABLE HANDSON.CAR_INSURANCE.POLICIES
ADD ROW ACCESS POLICY HANDSON.CAR_INSURANCE.RAP_POLICIES_BY_PREF ON (REGISTERED_PREF);

-- 4. 権限付与
GRANT USAGE ON DATABASE HANDSON TO ROLE SALES_TOKYO_ROLE;
GRANT USAGE ON DATABASE HANDSON TO ROLE SALES_OSAKA_ROLE;
GRANT USAGE ON SCHEMA HANDSON.CAR_INSURANCE TO ROLE SALES_TOKYO_ROLE;
GRANT USAGE ON SCHEMA HANDSON.CAR_INSURANCE TO ROLE SALES_OSAKA_ROLE;
GRANT SELECT ON TABLE HANDSON.CAR_INSURANCE.POLICIES TO ROLE SALES_TOKYO_ROLE;
GRANT SELECT ON TABLE HANDSON.CAR_INSURANCE.POLICIES TO ROLE SALES_OSAKA_ROLE;

-- 5. ユーザーへのロール付与 (CURRENT_USER() で動的に取得)
SET MY_USER = CURRENT_USER();
GRANT ROLE SALES_TOKYO_ROLE TO USER IDENTIFIER($MY_USER);
GRANT ROLE SALES_OSAKA_ROLE TO USER IDENTIFIER($MY_USER);

-- 6. (必要に応じて) ウェアハウス権限付与
-- GRANT USAGE ON WAREHOUSE COMPUTE_WH TO ROLE SALES_TOKYO_ROLE;
-- GRANT USAGE ON WAREHOUSE COMPUTE_WH TO ROLE SALES_OSAKA_ROLE;

-- =============================================
-- MASKING POLICY セットアップ
-- DX_ROLE は指定列が「*」/ NULL でマスクされる
-- =============================================

-- 7. DX_ROLE 作成
CREATE ROLE IF NOT EXISTS DX_ROLE;

-- 8. データ型別マスキングポリシー作成
CREATE OR REPLACE MASKING POLICY HANDSON.CAR_INSURANCE.MASK_DX_VARCHAR
AS (val VARCHAR) RETURNS VARCHAR ->
  CASE WHEN CURRENT_ROLE() = 'DX_ROLE' THEN '*' ELSE val END;

CREATE OR REPLACE MASKING POLICY HANDSON.CAR_INSURANCE.MASK_DX_NUMBER
AS (val NUMBER) RETURNS NUMBER ->
  CASE WHEN CURRENT_ROLE() = 'DX_ROLE' THEN NULL ELSE val END;

CREATE OR REPLACE MASKING POLICY HANDSON.CAR_INSURANCE.MASK_DX_DATE
AS (val DATE) RETURNS DATE ->
  CASE WHEN CURRENT_ROLE() = 'DX_ROLE' THEN NULL ELSE val END;

-- 9. POLICIES テーブルの指定列にマスキングポリシー適用
ALTER TABLE HANDSON.CAR_INSURANCE.POLICIES MODIFY
  COLUMN ANNUAL_PREMIUM_YEN SET MASKING POLICY HANDSON.CAR_INSURANCE.MASK_DX_NUMBER,
  COLUMN DEDUCTIBLE_COLLISION_YEN SET MASKING POLICY HANDSON.CAR_INSURANCE.MASK_DX_NUMBER,
  COLUMN DRIVER_AGE SET MASKING POLICY HANDSON.CAR_INSURANCE.MASK_DX_NUMBER,
  COLUMN LICENSE_YEARS SET MASKING POLICY HANDSON.CAR_INSURANCE.MASK_DX_NUMBER,
  COLUMN NCB_YEARS SET MASKING POLICY HANDSON.CAR_INSURANCE.MASK_DX_NUMBER,
  COLUMN POLICY_END_DATE SET MASKING POLICY HANDSON.CAR_INSURANCE.MASK_DX_DATE,
  COLUMN POLICY_START_DATE SET MASKING POLICY HANDSON.CAR_INSURANCE.MASK_DX_DATE,
  COLUMN POLICY_STATUS SET MASKING POLICY HANDSON.CAR_INSURANCE.MASK_DX_VARCHAR,
  COLUMN PRIOR_CLAIMS_3Y SET MASKING POLICY HANDSON.CAR_INSURANCE.MASK_DX_NUMBER;

-- 10. DX_ROLE に権限付与
GRANT USAGE ON DATABASE HANDSON TO ROLE DX_ROLE;
GRANT USAGE ON SCHEMA HANDSON.CAR_INSURANCE TO ROLE DX_ROLE;
GRANT SELECT ON TABLE HANDSON.CAR_INSURANCE.POLICIES TO ROLE DX_ROLE;

-- 11. DX_ROLE をユーザーに付与
GRANT ROLE DX_ROLE TO USER IDENTIFIER($MY_USER);
